...
description: This page's goal is to present a PostHoc feature attribution XAI methodology called LIME (Local Interpretable Model-agnostic Explanations) and how it can be used to explain image classification tasks. You will be guided through the code and the results of the LIME algorithm. Part of this workshop will be some research questions that need to be answered by you. These questions can be found all over this Blogspot using the <mark>TOSUBMIT</mark> tag and will be summarized at the end of the blogspot.
date: 2023-05-13
htmlwidgets: true
...
The above equation contains the tradeoff between <en>local fidelity</em> which is expressed by $L$ and <em>complexity</em> that it is expressed by $\Omega$.
...
<center>
<video autoplay muted loop controls src="{{ site.url }}{{ site.baseurl }}/assets/video/2023-05-13-LIME/LIME.mp4"
    style="width:600px"
    type="video/mp4">
</video>
<figcaption>A demonstration of the whole LIME algorithm.</figcaption>
...
Then, we apply our <mark style="background-color:Lavender;">black-box</mark> model $g()$ and we can extract the label for all the perturbations (and can be seen with red that denotes turtle and purple that denotes non-turtle). The <em>importance</em> of each perturbation is determined by measuring its distance from the original instance to be explained. These distances are converted to weights by mapping the distances to a zero-one scale using a kernel function (see the color scale for the weights). All this information: the newly generated dataset, its class predictions and its weights are used to fit a simpler model, such as a linear model (gray line), that can be interpreted. The coefficients for the case of a linear model, are then extracted and used as the explanation for the prediction of the instance to be explained. The higher the coefficient, the more important the feature is for the prediction.  

{% include figure.html path="assets/img/2023-05-13-LIME/LIME.png" class="img-fluid" %}

## Creating Perturbations of image

In the case of image explanations, perturbations will be generated by turning on and off some of the superpixels in the image.
...
# Load images
# write a funtion to load images from path to tensor

```

Now you will need to make use of the LIME algorithm as we have shown you before and explain these images. Firstly, find the predictions of the classifier for each of the images. Then, use LIME to explain the predictions of the classifier. 

```python
# Load the binary classifier
model = torch.load('model/hus_wolf_model.pth')
...
As a <em>lab exercise</em>, and by using the LIME algorithm established in previous cells, you should explain the behavior of the classifier. You should the provided images in <mark>data/test/</mark> folder. <mark style="background-color:Lavender;"> Add your conclusions about the model $f()$. What are the main problems with this classifier?</mark> 
