...
# Working with an example from the paper
In this part of the tutorial we will make use of a classifier that is trained to classify images of wolves and huskeys. The classifier is a black-box so we do not have access to it. We have zero knolwedge of the training dataset but also of the classifier's architecture. We will use LIME to explain the classifier's predictions and try to understand what the classifier has learned. 
We will need to explain the behaviour of the classifier using images from huskeys of wolves. You can download images online and use them for this tutorial. 
